...
#    X_train = PCA(n_components=38).fit_transform(X_train)
    ...
    #X_train = anova_filter.fit_transform(X_train)
    X_train = anova_filter.fit_transform(X_train,y_train)

    onevsrest = OneVsRestClassifier(SVR(kernel='rbf',C=10,epsilon=.01,gamma=.01),n_jobs=-1).fit(X_train,y_train)
    onevsone = OneVsOneClassifier(SVR(kernel='rbf',C=10,epsilon=.01,gamma=.01),n_jobs=-1).fit(X_train,y_train)
