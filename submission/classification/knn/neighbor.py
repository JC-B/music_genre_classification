...
def loadData(file, trainSet, testSet, split):   
    rawData = pd.read_csv('out.csv', sep='|')
...
def getNearestNeighbor(trainSet, testValue, k):
    ...
    #sort the array and initialize neigbor
...
def getVotes(neighbors):
    ...
    for i in range(neigborLength):
        ...
        #keep the vote the same otherwise
            votes[hit] = 1
    #sort the array
    votesSorted = sorted(votes.iteritems(), key = operator.itemgetter(1), reverse = True)
...
def main():
    ...
    predicted = []
    mySplit = 0.67
    loadData('out.csv', trainSet, testSet, mySplit)
